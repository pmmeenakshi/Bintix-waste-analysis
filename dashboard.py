...
with st.sidebar:
    ...
    show_popup_charts = st.toggle("Show charts in popups (slower)", value=False, help="Renders mini charts inside each popup. Turn off for fast map updates.")
...
with tab_map:
    ...
    if not has_latlon:
        ...
        for comm, pin, lat, lon, city in zip(comm_arr, pin_arr, lat_arr, lon_arr, city_arr):
            ...
            # Only render popup charts if the toggle is ON (much faster when OFF)            bar_img = donut_img = ""
            # Use image-based charts (safe for Folium popups)
            # Only render popup charts if the toggle is ON (much faster when OFF)
            bar_img, donut_img = "", ""   # <-- ensure defined even when toggle is OFF
